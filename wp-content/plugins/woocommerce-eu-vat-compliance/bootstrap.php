...
class WC_EU_VAT_Compliance {
	...
	public function ajax_update_checkout_totals($form_data) {
		...
		// $wc_vat_ip_country_code = empty($country_info['data']) ? '??' : $country_info['data'];
	...
	public function get_customer_taxable_region_code() {
		...
		$taxation_country = isset($taxable_address[0]) ? $taxable_address[0] : $this->wc->countries->get_base_country();
	...
	public function woocommerce_check_cart_items() {
		...
		if (!$this->cart_is_permitted()) {
			...
			if ('woocommerce_checkout_process' != $current_filter && (!defined('WOOCOMMERCE_CHECKOUT') || !WOOCOMMERCE_CHECKOUT)) {
				// Cart: just warn
				echo "<p class=\"woocommerce-info\" id=\"wcvat_notpossible\">".apply_filters('wceuvat_euvatcart_message', sprintf(__('Depending on your country, it may not be possible to purchase all the items in this cart. This is because this store does not sell items liable to VAT to customers in %s, due to the costs of complying with VAT laws.', 'woocommerce-eu-vat-compliance'), $region_object->get_region_title('noun')))."</p>";
			} else {
	...
	/**
	 * Get the taxable address for the current customer
	 *
	 * @return Array
	 */
...
// 			$taxes[$order_tax_id]['tax_rate_state'] = $tax_rate_info['tax_rate_state'];
	...
	private function version_1_19_7_rename_valid_eu_vat_meta_field() {
		...
		if (false === $result) {
			error_log("WooCommerce VAT compliance: version_1_19_7_rename_valid_eu_vat_meta_field() failed");
		} elseif ($result > 0) {
			error_log("WooCommerce VAT compliance: $result meta fields were updated ('Valid EU VAT Number' -> 'Valid VAT Number')");
		}
	...
	public function plugins_loaded() {
		...
		$this->settings[] = array(
			...
			'desc' => __('A comma-separated (optional spaces) list of strings (phrases) used to identify taxes which are variable-by-country VAT taxes. One of these strings must be used in your tax name labels (i.e. the names used in your tax tables) if you wish the tax to be identified as variable-by-country VAT.', 'woocommerce-eu-vat-compliance'),
