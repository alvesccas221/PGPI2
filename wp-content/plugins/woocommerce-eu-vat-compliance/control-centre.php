			...
			foreach ($all_settings as $setting) {
				...
				if (!isset($posted_settings[$setting['id']])) {
					if (!isset($setting['value_when_missing'])) {
						...
						// Currently only used for one option which is an array
...
// .' - '.sprintf(__('version %s', 'woocommerce-eu-vat-compliance'), $version);
	...
	public function get_settings_vat() {
		$vat_settings = array(
			array('title' => __('WooCommerce VAT settings (new settings from the VAT compliance plugin)', 'woocommerce-eu-vat-compliance'), 'type' => 'euvat_tax_options_section', 'desc' => '', 'id' => 'euvat_vat_options'),
		);
	...
	public function get_settings_tax() {
		...
		$tax_settings = array(
			...
			array('title' => __('Other WooCommerce tax options potentially relevant for VAT compliance', 'woocommerce-eu-vat-compliance'), 'type' => 'euvat_tax_options_section', 'desc' => '', 'id' => 'euvat_tax_options'),
	...
	private function render_tab_settings() {
		...
		echo '<p><a href="hhttps://ec.europa.eu/taxation_customs/sites/taxation/files/resources/documents/taxation/vat/how_vat_works/rates/vat_rates_en.pdf">'.__('Official EU documentation on current VAT rates (as of June 2021 giving an error with no apparent official replacement; hopefully they will soon fix it).', 'woocommerce-eu-vat-compliance').'</a></p>';
		...
		<script>
			...
			jQuery(function($) {
				...
				$('#euvatcompliance-export-settings').on('click', function(e) {
					e.preventDefault();
	...
	public function admin_footer() {
		...
		$text2 = esc_attr(__('N.B. The WooCommerce VAT Compliance plugin causes geo-location to identify the default address, regardless of whether you also activate the geo-location built into WooCommerce here. We recommend choosing "Shop Base Address" here (though, choosing "Geolocate address" should be harmless, as both geo-locations should have the same result).', 'woocommerce-eu-vat-compliance'));
